...
void LfoComponent::init() {
  _multiplier = 2.0f * (float)M_PI / Module::belaContext->audioSampleRate;
  _twoPi = 2.0f * (float)M_PI;
}

void LfoComponent::update(unsigned int n) {
  float noteNum = 12.0 * inputs[0] + 60.0;
  float newFreq = powf_neon(2.0, (noteNum - 69)/12.0) * 0.43; // later only change if input changes, for speed (pretty big saving)
